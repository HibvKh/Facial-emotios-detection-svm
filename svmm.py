...
general_path = r"C:\Users\RPC\Desktop\detectionml\datasetML1\archive (10)"
...
# Function to detect emotions from video stream
def detect_emotion():
    while True:
...
# Streamlit UI
st.title("Détection des émotions faciales en direct")

# Display an image on the homepage
image_path = r'C:\Users\RPC\Desktop\detectionml\1-principales-expressions-visage.jpg'  # Replace with your image path
st.image(image_path, caption="WELCOME!", use_column_width=True)

# Add a sidebar with options
option = st.sidebar.selectbox("choix", ["Accueil", "Détectez votre émotion", "À propos"])

# Display content based on selected option
if option == "Accueil":
    st.write("Bienvenue sur notre application de détection des émotions faciales en direct!")

elif option == "Détectez votre émotion":
    st.write("Ouvrez votre webcam pour détecter votre émotion en direct.")
    detect_emotion()

else:  # About section
    st.write("À propos de cette application...")
    st.write("Cette application utilise un modèle de détection des émotions faciales pour analyser les expressions en temps réel à partir de la caméra.")
    st.write("Le modèle est entraîné pour reconnaître 7 émotions différentes : colère, surprise, dégoût, peur, joie, neutralité et tristesse.")
